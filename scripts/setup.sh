...
# nothing: it only says so when this clone's hooks are not the snapshot this
# script installs — a fresh clone, or one still wired to the Husky hooks this
# repository used before. Installing from `prepare` instead would run the
...
# the pattern that named only the importers. (The server repo learned this;
# the list here starts with the glob.)
#
...
#
# Husky, which this replaces, pointed `core.hooksPath` at the tracked
# `.husky/` directory. That makes the hook SCRIPT branch-controlled code: a pull request can rewrite `pre-commit`, and a
...
# pins WHICH script runs, not WHAT it runs. `pre-commit` execs
# `pnpm exec lint-staged` and `pre-push` execs `pnpm verify:fast`, and both
# resolve through the WORKING TREE: the branch's `package.json`, its lint
# configs, its tests. A gate that checked anything else would not be a gate.
#
# Nor can the comparison above reach it: that guard runs now, at install time,
# and the commit that hands a branch's lint config to pnpm happens later, in a
...
#
# Node is optional on this leg only in principle — nothing else in this
# repository runs without it — but a missing node must still leave the git
# hooks installed, so this reports and moves on instead of failing.
