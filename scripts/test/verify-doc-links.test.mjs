...
test('a link inside inline code is a literal string nobody can click', () => {
  withTree(
    {
      'CLAUDE.md':
        'A typo row looks like `[board](board/TYPO.md)` in the index.',
    },
    (root) => assert.deepEqual(collectFindings(root), []),
  );
});
  ...
  withTree(
    {
      ...
      'docs/tasks/archive/2026/08/done-todo.md':
        'Cited [a design](../../../../design/gone.md) that has since moved.',
    },
    (root) => assert.deepEqual(collectFindings(root), []),
  );
});
  ...
  withTree(
    {
      ...
      'docs/tasks/archive/README.md':
        '[a finished task](2026/08/missing-todo.md)',
    },
    (root) => {
      const findings = collectFindings(root);
      assert.equal(findings.length, 1);
      assert.match(
        findings[0],
        /archive\/README\.md links 2026\/08\/missing-todo\.md/,
      );
    },
  );
});
