...
export const HOOK_WIRING = [
  { event: 'SessionStart', matcher: '', script: 'session-prime.sh' },
  {
    event: 'PreToolUse',
    matcher: 'Edit|Write',
    script: 'guard-generated-files.sh',
  },
];
...
function isOurs(command) {
  return (
    ...
    new RegExp(
      `(^|[\\s/'"])(${SNAPSHOT_DIRNAME}|scripts/hooks)/[\\w.-]+\\.sh(['"\\s]|$)`,
    ).test(command)
  );
}
    ...
    hooks[event] = [
      ...kept,
      {
        ...
        hooks: [
          {
            type: 'command',
            command: `bash ${shellQuote(path.join(snapshotDir, script))}`,
          },
        ],
      },
    ];
...
function main() {
  ...
  if (!root || !gitDir) {
    process.stderr.write(
      'not a git checkout; skipping Claude Code hook install\n',
    );
    process.exit(1);
  }
  ...
  if (existsSync(settingsPath)) {
    ...
    try {
      ...
      process.stderr.write(
        `${settingsPath} is not valid JSON (${err.message}); left alone\n`,
      );
      process.exit(1);
    }
  }
  ...
  writeFileSync(
    settingsPath,
    `${JSON.stringify(wireHooks(existing, snapshotDir), null, 2)}\n`,
  );
