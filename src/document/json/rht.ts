...
export class RHT {
  ...
  public set(key: string, value: string, movedAt: TimeTicket): void {
    const prev = this.nodeMapByKey.get(key);

    if (prev === undefined || movedAt.after(prev.getMovedAt())) {
      const node = RHTNode.of(key, value, movedAt);
      this.nodeMapByKey.set(key, node);
      this.nodeMapByCreatedAt.set(movedAt.toIDString(), node);
    }
  }
