...
describe('Array', function () {
  it('should handle delete operations', function () {
    const doc = createTestDocument<{ k1: JSONArray<string> }>('test-doc');
  ...
  it('can push array element after delete operation', function () {
    const doc = createTestDocument<{ k1: JSONArray<string | Array<number>> }>(
      'test-doc',
    );
    assert.equal('{}', doc.toSortedJSON());
  ...
  it('can push array', function () {
    const doc = createTestDocument<{ arr: Array<number | Array<number>> }>(
      'test-doc',
    );
  ...
  it('can push element then delete it by ID in array', function () {
    const doc = createTestDocument<{ list: JSONArray<number> }>('test-doc');
    assert.equal('{}', doc.toSortedJSON());
