    ...
    <div className="app">
      <header>
        ...
        <p className="lede">
          ...
          post" button reads a separate <code>writers</code> channel in{' '}
          <code>readOnly</code> mode — its count rises only when someone opens
          the composer route.
        </p>
      </header>
      ...
      <footer>
        Two scenarios in one playground:
        <ul>
          <li>
            <strong>Sub-route flicker</strong>: toggle{' '}
            <code>Provider position</code> and click between{' '}
            <code>/overview</code> ↔ <code>/activity</code>. With{' '}
            <code>inside</code>, the other tab sees the stock count blink to{' '}
            <code>0</code> after each navigation; with <code>outside</code>,
            the blink disappears.
          </li>
          <li>
            <strong>readOnly attach</strong>: click <code>Write a post</code>{' '}
            in one tab → the other tab's <code>writing</code> badge ticks up.
            The viewer is <em>not</em> counted in that badge thanks to{' '}
            <code>readOnly</code> attach.
          </li>
        </ul>
      </footer>
    </div>
