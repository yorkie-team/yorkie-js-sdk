...
**On `compositionstart`**: Set `isComposing = true`, capture `composingBlockRange` (the PM position range of the top-level block containing the selection), and put the document into `SyncMode.RealtimePushOnly`. Push-only is deliberate: local edits keep reaching peers while the user composes, matching the CodeMirror and Quill bindings, while the client still refuses every incoming change (the request carries `pushOnly`, and a response pack that arrives anyway is dropped). Only the *apply* direction threatens the composing text node, so only it needs to stop.
