...
- Replay events and notifications are both held in memory, keyed by document key, so a long editing session grows without bound. `setupDevtools` notes that external storage such as IndexedDB should replace this.
- `isStatusRepeat` collapses a notification only against the most recent record of the same type, so the two `AuthError` streams it exists to suppress — one from the sync loop with `method: 'PushPull'`, one from the watch loop with `method: 'Watch'` — alternate past it and are all recorded.
